	...
	return (
		...
		(function (k, U, X, s) {
			...
			function Wa(f) {
				return (
					f.touch ||
					(f.originalEvent.touches && f.originalEvent.touches[0]) ||
					f
				)
			}
			function vb(f) {
			...
			if (k) {
				...
				if (!Z || +(xb(Z[0]) + xb(Z[1]) + xb(Z[2] || '')) < 10602)
					return Ab(
						'Too old jQuery library. Please upgrade your jQuery to version 1.6.2 or higher'
					)
				var n = (k.reel = {
						version: '1.3.0',
						def: {
						...
						scan: function () {
							return k(x(z) + ':not(' + x(Bb) + ' > ' + x(z) + ')').each(
								function (f, j) {
									f = k(j)
									j = f.data()
									j.images = yb(j.images)
									var o = {}
									k(x(dc) + '[data-for=' + f.attr(ka) + ']').each(function (
										t,
										r
									) {
										t = k(r)
													...
													instance: {
														...
														'setup.annotations': function () {
															...
															k.each(a(Pa), function (d, b) {
																var i =
																	typeof b.node == wb ? k(b.node) : b.node || {}
				...
				var ad = !(Ba && Z < 8),
					...
					Ac = { W: D, H: L },
					uc = ad
						? Dc('CAAIAIAAAAAAAAAAACH5BAEAAAAALAAAAAAIAAgAAAIHhI+py+1dAAA7')
						: Da + 'blank.gif',
